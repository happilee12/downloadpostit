...
try {
  ...
  const emptyFileContent = JSON.stringify({"todo" : [], "deleted": [], "completed" : []})
...
async function createWindow() {
  ...
  ipcMain.on('addToTodosAndSave', (event, targetPostit) => {
  ...
  ipcMain.on('addToDeletedAndSave', (event, targetPostit) => {
    appData.deleted.push(targetPostit)
    fs.writeFileSync(datapath, JSON.stringify(appData));
  })
  ...
  ipcMain.on('addToCompletedAndSave', (event, targetPostit) => {
    appData.completed.push(targetPostit)
    fs.writeFileSync(datapath, JSON.stringify(appData));
  })
}
